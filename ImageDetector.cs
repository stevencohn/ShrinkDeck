...
namespace ShrinkDeck
{
	...
	internal class ImageDetector
	{
		...
		public ImageDetector()
		{
			...
			JpgHead = [0xFF, 0xD8, 0xFF];							// JPEG JFIF (SOI "\xFF\xD8" and half next marker xFF)
			JpgTail = [0xFF, 0xD9];									// JPEG EOI "\xFF\xD9"
			PngHead = [0x89, 0x50, 0x4e, 0x47, 0x0D, 0x0A, 0x1A, 0x0A]; // PNG "\x89PNG\x0D\0xA\0x1A\0x0A"
			TiffIHead = [0x49, 0x49, 0x2A, 0x00];					// TIFF II "II\x2A\x00"
			TiffMHead = [0x4D, 0x4D, 0x00, 0x2A];					// TIFF MM "MM\x00\x2A"
		}
		...
		public ImageSignature GetSignature(byte[] buffer)
		{
			try
			{
